...
var mangaSite = {
	...
	getComicDetail : function (e,callback){
		/* * /
	var comic = {};
		a = e.find("a:first-of-type"),
		b = a.attr("href"),
		c = e.find("p:first-of-type").html();
		/* */
	var	comic = { 
			url : e.a[0].href,
			fullUrl : LS._TARGETSITE.url + e.a[0].href,
			id : /\d+/ig.exec(e.a[0].href).toString(),
			title : e.a[0].title,
			coverImg : e.a[0].img.src,
			lastestEps : /\d+/ig.exec(e.p).toString()
			}	
		console.log(comic);
		callback(comic)
	},
	...
	doLastest : function(){
		LS.requestCrossDomain(LS._TARGETSITE.url,LS._TARGETSITE.xpath, function doResult(results) {
			...
			while(i--) LS.getComicDetail(obj[i],function(comic){
						var divEntry = $("<div/>", {id:comic.id,class:"c-entry",click: LS.doLoadEps}).data(comic),
	...
	//	console.log(event.currentTarget.getComic());
...
function doConcatUrl(obj){
	var phaseURL = _TARGETSITE.url.concat($(obj).attr("rel"));
	console.log("doConcatUrl = "+phaseURL);
	return phaseURL;
	}
	
function doAjaxLoadEp(url){
	requestCrossDomain(url,_TARGETSITE.xpath2, function(results) {
...
function listPg(){
	...
	var phaseURL = _TARGETSITE.url.concat(linkTo);
